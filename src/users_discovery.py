...
from src.utils import get_online_users, load_all_users
...
class UserDirectory(App):
    ...
    async def get_online_users_once(self):
        ...
        try:
            ...
            self.update_list()
            logger.debug("Online users updated: %d users", len(online_users))
        except Exception as e:
            logger.warning("Failed to fetch online users: %s", e)
            self.notify("Failed to fetch online users", severity="warning")
    ...
    def update_list(self, search=""):
        ...
        filtered_users = [
            user for user in self.users if search.lower() in user["username"].lower()
        ]
        ...
        for user in filtered_users:
            ...
            if (
                self.selected_user
                and user["username"] == self.selected_user["username"]
            ):
    ...
    def update_status(self):
        ...
        if self.selected_user:
            ...
            status = "Online" if is_online else "Offline"
            ip_info = (
                f" - IP: {self.selected_user.get('ip', 'Unknown')}" if is_online else ""
            )
            status_display.update(
                f"Selected: {self.selected_user['username']} ({status}{ip_info})"
            )
        else:
    ...
    async def on_button_pressed(self, event):
        if event.button.id == "refresh":
            ...
            if self.selected_user:
                ip = self.selected_user.get("ip", "Unknown")
        ...
        if event.input.id == "search":
            ...
            if self.selected_user:
                filtered_users = [
                    ...
                    if current_search.lower() in user["username"].lower()
                ]
